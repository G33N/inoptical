...
function entregarTarjeta(id){
  $.ajax({
      type: 'POST',
      url: "datos/ameBeneficios.php",
      data: $(this).serialize() +"&orden="+21+"&id="+id,
      success:function(data){
              if(data.success === true ){
                  listarRegistrados(1);
                  alert(data.message);
...
function filtarRegistrados(){
  $('#pendientes').click(function(){
    listarRegistrados(1);
  });
  $('#entregados').click(function(){
    listarRegistrados(0);
  });
}
function listarRegistrados(estado){
$.ajax({
        type: 'POST',
        url: "datos/ameBeneficios.php",
        data: $(this).serialize() +"&orden="+0 +"&estado="+estado,
        success: function(data){
